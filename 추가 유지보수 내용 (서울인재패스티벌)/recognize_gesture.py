...
def put_splitted_text_in_blackboard(blackboard, splitted_text):
        y = 0
        img1 = np.zeros((1100,1980,3),np.uint8)
...
def recognize():
        ...
        cur_text = ""   #현재 인식된 텍스트
        cur_text_box = [] # 누적시키는 텍스트 구분을 위한 리스트
        cur_sentence = "" #인식된 텍스트 누적
        ...
        while True:
                ...
                blackboard = np.zeros((600, 0, 3), dtype=np.uint8)
                ...
                if cur_text == '':
                        ...
                        if keep_count == 10:
                                ...
                                if not cur_text_box or cur_text_box[-1] != cur_text:
                                        cur_text_box.append(cur_text)
                                        cur_sentence += cur_text
                                        if cur_text == '.':
                                                display_sentence.append(cur_sentence)
                                                cur_sentence = ''
                                if len(display_sentence) > 10:
		...
		#cv2.putText(blackboard, text, (30, 200), cv2.FONT_HERSHEY_TRIPLEX, 1.3, (255, 255, 255))
                ...
                cv2.imshow("Recognizing test", res)
